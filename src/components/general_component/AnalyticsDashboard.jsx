...
import {
  ...
  Calendar, FileText, Target, Activity, PieChart as PieChartIcon, 
  ArrowUpRight, BarChart3, Layers, ClipboardCheck
} from "lucide-react";
...
export default function AnalyticsDashboard({ data = [] }) {
  const analytics = useMemo(() => {
    ...
    // --- CONTEOS BÁSICOS ---
    // Verificamos propiedad snake_case (DB) o camelCase (Frontend)
    const completed = data.filter((d) => d.actual_issue_date || d.actualIssueDate).length;
    
    const inReview = data.filter((d) => 
      (d.latest_review_status === "IN_REVIEW" || d.lastReviewStatus === "IN_REVIEW")
    ).length;
    
    const approved = data.filter((d) => 
      (d.latest_review_status === "APPROVED" || d.lastReviewStatus === "APPROVED")
    ).length;
    
    const pending = total - completed;
    const withApprovalFlow = data.filter((d) => d.has_approval_flow || d.hasApprovalFlow).length;
    
    // --- CÁLCULO DE RETRASOS Y GENERACIÓN DE DATOS PARA SCATTER ---
    let onTimeCount = 0;
    let delayedCount = 0;
    let earlyCount = 0;
    const delayDays = [];
    const genDeviationData = []; // Para Scatter Chart
    let totalGenDelay = 0;
    let genCount = 0;
    ...
    data.forEach((item) => {
        ...
        // 1. Cálculo de Desviación de Emisión (KPIs)
        if (pIssue && aIssue) {
        ...
        // 2. Cálculo de Desviación de Generación (Scatter Plot)
        if (pGen && aGen) {
    ...
    // --- AGRUPACIÓN POR MES (TIMELINE EMISIÓN & REVISIÓN) ---
    // Usaremos un diccionario para acumular todo por mes
    const timelineMap = {};
    ...
    // Convertir timeline a array ordenado
    const timelineArray = Object.values(timelineMap).sort((a,b) => a.month.localeCompare(b.month));

    // Calcular ACUMULADOS para Curva S
    let accP = 0;
    let accA = 0;
    const sCurveData = timelineArray.map(t => {
        accP += t.pIssue;
        accA += t.aIssue; // Nota: Esto asume que aIssue sucedió en el mismo mes programado, para mayor precisión habría que mapear aIssue a su propio mes real, pero para dashboard visual esto suele bastar o se ajusta la lógica de agrupación.
        return {
            month: t.month,
            plannedAcc: accP,
            actualAcc: accA,
            ...t // Mantenemos los valores mensuales individuales
        };
    });

    // --- AGRUPACIÓN POR CONJUNTO (VERSION SET) ---
    const byVersionSetMap = data.reduce((acc, item) => {
        const set = item.sheet_version_set || item.issueVersionSetName || "Sin asignar";
        if (!acc[set]) {
            acc[set] = { name: set, total: 0, completed: 0, inReview: 0, pending: 0 };
        }
        acc[set].total++;
    ...
    return {
      total, completed, inReview, approved, pending, withApprovalFlow,
      onTimeCount, delayedCount, earlyCount, avgDelay, avgGenDelay,
      completionRate: total > 0 ? Math.round((completed / total) * 100) : 0,
      
      genDeviationData,
      timelineArray, // Para gráficos mensuales
      sCurveData,    // Para acumulados
      byVersionSet,
      revisionData
    };
  }, [data]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <Card className="bg-emerald-50/50 border-emerald-100">
          <CardContent className="p-4">
            ...
            <div className="mt-2 text-[10px] text-emerald-600 font-medium">
                {analytics.completionRate}% del total
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-violet-50/50 border-violet-100">
          <CardContent className="p-4">
            <div className="flex justify-between items-center">
              ...
              <div className="p-2 bg-violet-100 rounded-lg"><Target className="h-5 w-5 text-violet-600" /></div>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Card: Desviación Promedio (Emisión) */}
        <Card className={`${analytics.avgDelay > 0 ? "bg-red-50/50 border-red-100" : "bg-green-50/50 border-green-100"}`}>
           <CardContent className="p-4">
            <div className="flex justify-between items-center">
              <div>
                <p className={`text-xs font-medium ${analytics.avgDelay > 0 ? "text-red-600/80" : "text-green-600/80"}`}>
                    Desv. Entrega
                </p>
                <p className={`text-2xl font-bold ${analytics.avgDelay > 0 ? "text-red-700" : "text-green-700"}`}>
                    {analytics.avgDelay > 0 ? "+" : ""}{analytics.avgDelay}d
                </p>
              </div>
              <div className={`p-2 rounded-lg ${analytics.avgDelay > 0 ? "bg-red-100" : "bg-green-100"}`}>
                {analytics.avgDelay > 0 ? <TrendingDown className="h-5 w-5 text-red-600" /> : <TrendingUp className="h-5 w-5 text-green-600" />}
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Card: Desviación Generación */}
        <Card className={`${analytics.avgGenDelay > 0 ? "bg-red-50/50 border-red-100" : "bg-green-50/50 border-green-100"}`}>
           <CardContent className="p-4">
            <div className="flex justify-between items-center">
              <div>
                <p className={`text-xs font-medium ${analytics.avgGenDelay > 0 ? "text-red-600/80" : "text-green-600/80"}`}>
                    Desv. Generación
                </p>
                <p className={`text-2xl font-bold ${analytics.avgGenDelay > 0 ? "text-red-700" : "text-green-700"}`}>
                    {analytics.avgGenDelay > 0 ? "+" : ""}{analytics.avgGenDelay}d
                </p>
              </div>
              <div className={`p-2 rounded-lg ${analytics.avgGenDelay > 0 ? "bg-red-100" : "bg-green-100"}`}>
                <Activity className={`h-5 w-5 ${analytics.avgGenDelay > 0 ? "text-red-600" : "text-green-600"}`} />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Evolución Mensual (Area - No acumulada) */}
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
      ...
      {/* 3. GRÁFICOS DE DESEMPEÑO (FILA 2 - NUEVOS) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* Desviación de Generación (Scatter) */}
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
            ...
            <div className="h-[300px] w-full">
                <ChartContainer config={chartConfig} className="h-full w-full">
                    <AreaChart data={analytics.sCurveData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                        ...
                        <ChartTooltip content={<ChartTooltipContent />} />
                        <Legend />
                        <Area type="monotone" dataKey="plannedAcc" name="Prog. Acumulado" stroke="#3b82f6" fill="transparent" strokeWidth={2} />
                        <Area type="monotone" dataKey="actualAcc" name="Real Acumulado" stroke="#10b981" fill="transparent" strokeWidth={2} />
                    </AreaChart>
                </ChartContainer>
            </div>
          </CardContent>
                ...
                <ChartContainer config={{}} className="h-full w-full">
                    <BarChart data={analytics.revisionData} layout="vertical" margin={{ left: 20 }}>
                        ...
                        <ChartTooltip cursor={{fill: 'transparent'}} content={({active, payload}) => {
                            if(active && payload && payload.length) {
                                return (
                                    <div className="bg-popover border p-2 rounded text-sm">
                                        <p>Rev <strong>{payload[0].payload.name}</strong>: {payload[0].value} planos</p>
                                    </div>
                                )
                            }
                            return null;
                        }} />
                        <Bar dataKey="value" fill="#8b5cf6" radius={[0, 4, 4, 0]} label={{ position: 'right', fill: '#666' }} barSize={30} />
                    </BarChart>
                </ChartContainer>
            </div>
          </CardContent>
